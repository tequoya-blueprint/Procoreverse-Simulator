// --- app-controls.js ---
// VERSION: 72 (CUSTOM SCOPE LIST + SCOPING LOGIC)
...
const audienceDataToKeyMap = { "Contractor": "GC", "General Contractor": "GC", "GC": "GC", "SC": "SC", "Specialty Contractor": "SC", "Owners": "O", "Owner": "O", "Owner Developer *Coming Soon": "O", "GC": "GC", "SC": "SC", "O": "O" };
const audienceKeyToLabelMap = { "GC": "General Contractor", "SC": "Specialty Contractor", "O": "Owner" };
const audienceKeyToDataValuesMap = { "GC": ["Contractor", "General Contractor", "GC"], "SC": ["SC", "Specialty Contractor"], "O": ["Owners", "Owner", "Owner Developer *Coming Soon", "O"] };

function initializeControls() {
    if (typeof app !== 'undefined') app.customScope = new Set();
    if (typeof packagingData === 'undefined') { console.warn("Procoreverse: Packaging data missing."); return; }

    document.querySelectorAll('.accordion-header').forEach(header => {
        header.addEventListener('click', () => { if(typeof toggleAccordion === 'function') toggleAccordion(header.parentElement); });
    });

    populateRegionFilter(); populatePersonaFilter(); populateServiceAddons(); 
    ...
    if (!teamSelector.empty()) {
        ...
        if (TEAM_CONFIG[initialTeam]) { teamSelector.property('value', initialTeam); setTimeout(() => applyTeamView(initialTeam), 100); }
        teamSelector.on("change", function() { applyTeamView(this.value); });
    }

    d3.select("body").on("click", (e) => {
        if (e.target && !document.getElementById('search-container').contains(e.target)) { d3.select("#search-results").html("").style("opacity", 0).style("transform", "scale(0.95)"); }
    });
    ...
    inputs.forEach(id => { const el = document.getElementById(id); if(el) el.addEventListener(el.tagName === 'SELECT' ? 'change' : 'input', calculateScoping); });
}
...
function populateServiceAddons() {
    const addons = [{ name: "None", cost: 0 }, { name: "Analytics Implementation (10 hrs)", cost: 2500 }, { name: "ERP Integration Support (20 hrs)", cost: 5000 }, { name: "Custom Reporting (5 hrs)", cost: 1250 }, { name: "Legacy Data Migration", cost: 15000 }];
    const select = d3.select("#addon-select");
    if(select.empty()) return;
    select.html("");
    select.selectAll("option").data(addons).enter().append("option").text(d => d.name).attr("value", d => d.cost);
}
...
function calculateScoping() {
    ...
    if (!listContainer) {
        ...
        if(revenueContainer) {
            ...
            revenueContainer.parentNode.insertBefore(listContainer, revenueContainer);
        }
    }
    ...
    if (listContainer) {
        if (customToolCount > 0) {
            ...
            listContainer.innerHTML = `<div class="font-bold text-gray-500 mb-1">Custom Additions (${customToolCount}):</div><div class="text-indigo-600 leading-snug">${items}</div>`;
    ...
    if (baseLabel) {
        ...
        if (customToolCount > 0) labelText += ` + ${customHours} Hrs (Custom)`;
        if (addOnCount > 0) labelText += ` + ${addOnCount} Add-ons`;
        if (totalHoursRaw > 0) baseLabel.parentElement.innerHTML = `Base Scope: <span id="base-tools-count" class="font-bold text-gray-700">${labelText}</span>`;
        else baseLabel.innerText = "0 Hrs";
    }
    ...
    const serviceAddonCost = parseInt(addonSelect ? addonSelect.value : 0) || 0;
    const totalSOW = implementationCost + onsiteCost + serviceAddonCost;
...
function applyTeamView(team) {
    ...
    if (target) { target.classList.add('active'); target.querySelector('.accordion-content').style.maxHeight = target.querySelector('.accordion-content').scrollHeight + "px"; }
}
...
function populateRegionFilter() {
    ...
    packagingData.forEach(pkg => { if (pkg.region === 'NAM') regions.add('NAMER'); else regions.add(pkg.region); });
    [...regions].sort().forEach(region => {
        let label = region === "EUR" ? "EMEA" : region;
...
function onRegionChange() {
    ...
    if (region !== "all") {
        ...
        [...availableAudiences].sort().forEach(audKey => { audienceFilter.append("option").attr("value", audKey).text(audienceKeyToLabelMap[audKey]); });
    }
    if (typeof updateGraph === 'function') updateGraph(true);
}
...
function onAudienceChange() {
    ...
    const audienceDataKeys = audienceKeyToDataValuesMap[audience] || [];
    if (region !== 'all' && audience !== 'all') {
        const packages = packagingData.filter(pkg => (pkg.region === region || (region === 'NAMER' && pkg.region === 'NAM')) && audienceDataKeys.includes(pkg.audience));
        if (packages.length > 0) {
            ...
            packages.sort((a, b) => a.package_name.localeCompare(b.package_name)).forEach(pkg => {
                ...
                label.append("input").attr("type", "checkbox").attr("value", pkg.package_name).attr("class", "form-checkbox h-4 w-4 text-indigo-600 package-checkbox mr-2")
                    .on("change", () => { updatePackageAddOns(); if(typeof updateActivePackageState === 'function') updateActivePackageState(); if (typeof updateGraph === 'function') updateGraph(true); calculateScoping(); });
...
function updatePackageAddOns() {
    ...
    if (allAddOns.size > 0) {
        ...
        [...allAddOns].sort().forEach(addOn => {
            ...
            label.append("input").attr("type", "checkbox").attr("value", addOn).attr("class", "form-checkbox h-5 w-5 text-orange-600 transition rounded mr-3 focus:ring-orange-500")
    ...
    if (allServices.size > 0) {
        ...
        [...allServices].sort().forEach(service => { servicesList.append("div").attr("class", "flex items-center text-gray-700").html(`<i class="fas fa-check-circle text-green-500 mr-2"></i> ${service}`); });
...
function refreshAccordionHeight() {
    ...
    if (content && content.parentElement.classList.contains('active')) content.style.maxHeight = "1200px"; 
}
