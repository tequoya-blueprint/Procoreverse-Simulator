// --- app-main.js ---
// VERSION: 76 (SEMANTIC HEX CLUSTERS - "ORDER + VISIBILITY")
...
const app = {
    ...
    height: 0,
    categories: {}, 
    categoryFoci: {}, // Stores the center point for each Group
    baseNodeSize: 25,
    nodeSizeCompany: 28,
    nodeCollisionRadius: 45, // Radius to prevent overlap
    arrowRefX: 34, 
...
// --- NEW LAYOUT: SEMANTIC HEXAGONAL CLUSTERS ---
// Instead of a rigid grid for every node, we arrange the *Groups* in a Hexagon.
// Nodes float freely within their group zone, preventing spaghetti lines.
function setHexFoci() {
    ...
    // Radius of the "Ring of Groups"
    const radius = Math.min(app.width, app.height) * 0.35; 

    // Define the "Compass" for groups
    const layoutMap = {
        "Platform & Core": { angle: -1, dist: 0 }, // Center
        
        // Ring Order (Clockwise starting top)
        "Preconstruction": { angle: -90, dist: 1 },       // Top
        "Project Management": { angle: -30, dist: 1 },    // Top Right
        "Financial Management": { angle: 30, dist: 1 },   // Bottom Right
        "Construction Intelligence": { angle: 90, dist: 1 }, // Bottom
        "Resource Management": { angle: 150, dist: 1 },   // Bottom Left
        "Quality & Safety": { angle: 210, dist: 1 },      // Top Left (Mid)
        
        // Outer / Special
        "Helix": { angle: 90, dist: 0.6 }, // Inner Bottom
        "Project Execution": { angle: -30, dist: 0.6 }, // Inner Top Right
        "Workforce Management": { angle: 150, dist: 0.7 }, // Inner Left
        "Project Map": { angle: -60, dist: 0.5 },
        "External Integrations": { angle: 0, dist: 1.2 }, // Far Right
        "Emails": { angle: 180, dist: 1.1 } // Far Left
    };

    Object.keys(app.categories).forEach(cat => {
        const config = layoutMap[cat] || { angle: 0, dist: 0 };
        if (config.dist === 0) {
            app.categoryFoci[cat] = { x: cx, y: cy };
        } else {
...
// Custom Force: Pull nodes toward their Category Home
function forceCluster(alpha) {
    return function(d) {
        const focus = app.categoryFoci[d.group];
        if (!focus) return;
        // Alpha decay creates smooth entry
        const k = alpha * 0.15; 
        d.vx -= (d.x - focus.x) * k;
        d.vy -= (d.y - focus.y) * k;
    };
...
function initializeSimulation() {
    ...
    app.simulation = d3.forceSimulation()
        // 1. Links: Pull connected nodes together (Clarity)
        .force("link", d3.forceLink().id(d => d.id).distance(80).strength(0.4))
        // 2. Charge: Repel nodes to prevent clumping (-400 is medium repulsion)
        .force("charge", d3.forceManyBody().strength(-400))
        // 3. Collision: Strict no-overlap zone
        .force("collision", d3.forceCollide().radius(app.nodeCollisionRadius).strength(0.9))
        // 4. Center: Gravity to middle
...
function ticked() {
    // Apply Cluster Force manually during tick
